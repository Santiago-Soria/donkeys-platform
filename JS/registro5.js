document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupEventListeners() {
        ...
        backButton.addEventListener('click', () => {
            window.location.href = 'registro4.html'; // Ajusta la URL según tu estructura
        });
    ...
    function showErrorAlert(invalidFiles) {
        ...
        Swal.fire({
            ...
            html: `Error en los siguientes archivos:<br><br>${errorList}<br><br>Por favor sube solo archivos JPG, PNG o PDF menores a 10MB.`
        });
    }
    ...
    async function handleNextStep() {
        if (uploadedFiles.length < REQUIRED_FILES) {
            Swal.fire({
                ...
                text: `Por favor sube al menos ${REQUIRED_FILES} archivo${REQUIRED_FILES !== 1 ? 's' : ''} (frente y reverso del INE).`
            });
            return;
        }
